...
import CurrentRelease from '../../../components/CurrentRelease.astro';
...
- No `privileged` container. Neither the reverse proxy nor any application mounts the Docker socket: Traefik reads it through a filtering proxy that allows read calls only. The Docker-management tools are the documented exceptions — their proxies allow changes, and the Portainer and Hawser agents mount the socket directly
