...
<StackEvidence stack="core/onlyoffice"
...
Needs: [Traefik](/infrastructure/traefik/) running, a hostname, a JWT secret, and the origin(s) of each application that will embed the editor.
...
:::note
If the consuming application uses `acc-tailscale` but OnlyOffice connections are still rejected, the source IP Traefik sees may not match the expected Tailscale address — see [Traefik troubleshooting](/infrastructure/traefik/#troubleshooting) for the known Docker bridge source-IP caveat.
:::
