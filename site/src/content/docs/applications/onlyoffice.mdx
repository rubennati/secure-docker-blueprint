...
<StackEvidence stack="apps/onlyoffice"
  gap="opening a document from a consuming application. The health endpoints answer; the editor path through the iframe has not been run here." />
...
**To try it:** Docker, and a JWT secret, which the editor requires either way. See [Try it locally](#try-it-locally).

**To deploy it behind the proxy:** [Traefik](/infrastructure/traefik/) running, a hostname, a JWT secret, and the origin(s) of each application that will embed the editor.

## Try it locally

On your own machine, with no proxy, domain or certificate. From `apps/onlyoffice/`:
...
**Allowed-origins change has no effect:** `ONLYOFFICE_ALLOWED_ORIGINS` is baked into a Traefik label at container creation time, so `docker compose restart` does not update it. Run `docker compose up -d --force-recreate` on the OnlyOffice server.

**A request is rejected although the consuming application uses `acc-tailscale`:** the source IP Traefik sees may not match the expected Tailscale address — see [Traefik troubleshooting](/infrastructure/traefik/#troubleshooting) for the Docker bridge source-IP caveat.

**Private or Tailscale path: document does not load:** verify that DNS for the consuming application's hostname resolves to an address reachable from the OnlyOffice host. If it resolves to a public IP blocked by a firewall, the server-to-server fetch fails, and the browser console shows no clear error.

## Repository files

- [`apps/onlyoffice/README.md`](https://github.com/rubennati/secure-docker-blueprint/blob/main/apps/onlyoffice/README.md) — setup, JWT secret handling, iframe and CSP architecture
- [`apps/onlyoffice/UPSTREAM.md`](https://github.com/rubennati/secure-docker-blueprint/blob/main/apps/onlyoffice/UPSTREAM.md) — upgrade checklist, version notes, operational notes
- [`apps/onlyoffice/.env.example`](https://github.com/rubennati/secure-docker-blueprint/blob/main/apps/onlyoffice/.env.example) — all configurable variables with inline comments
