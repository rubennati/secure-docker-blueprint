...
The quickstart uses the default, `APP_TRAEFIK_ACCESS=acc-tailscale` — the vault is reachable only over Tailscale/VPN. Setting `APP_TRAEFIK_ACCESS=acc-public` makes it reachable from the open internet instead. That's a deliberate change to your attack surface, not a default — decide it before exposing real credentials.
