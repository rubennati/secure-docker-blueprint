...
<Tabs>
  <TabItem label="Wildcard">
    One DNS-01 certificate covers `*.example.com` — what the installation above sets up. Needs a Cloudflare account and token; does **not** need port 80 reachable from the internet. Best when several services share one domain, and avoids registering every subdomain in public Certificate Transparency logs.
  </TabItem>
