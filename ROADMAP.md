...
- An AI stack — model runtimes, chat interfaces, gateways, vector stores. Deploying and hardening one is in scope the moment a real need exists; assembling a catalogue of candidates in advance is not, and the engineering above the deployment line — model evaluation, retrieval architecture, prompt design — is a different project. See [`docs/architecture.md`](docs/architecture.md#ai--local-ai-is-latent-on-purpose).
- A general document-processing pipeline — extraction, layout and table recognition, structured output. The capabilities that exist are real (archive and OCR, browser editing, e-signature); a pipeline is defined by the document it handles and the output someone needs, so building one before a real case exists would produce an untestable chain of tools. See [`docs/architecture.md`](docs/architecture.md#document-processing-has-the-capabilities-not-a-pipeline).
