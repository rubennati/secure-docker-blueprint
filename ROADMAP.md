...
- The engineering above an AI deployment — model evaluation, retrieval architecture, prompt design. `apps/ollama`, `apps/vllm` and `apps/qdrant` deploy and harden the services; choosing models, designing retrieval and judging whether it works is a different project. See [`docs/architecture.md`](docs/architecture.md#ai--local-ai-is-deployment-not-engineering).
