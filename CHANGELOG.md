...
- **Two monitoring stacks, batch H of the 2026-09-22 candidates** (`monitoring/gotify/`, `monitoring/ciao/`). One container each, SQLite each, both `scaffolded`. **Gotify** is the second notification receiver beside ntfy: applications publish with their own token, clients hold a stream open. The administrator password comes from a Docker Secret and is read on the first start — without it upstream creates `admin`/`admin`. Registration is off, `GOTIFY_PLUGINSDIR` is empty so no compiled plugin is loaded from the data volume, the session cookie carries `Secure`, and the trusted-proxy setting puts the client's address in the access log instead of Traefik's. 3.1.1 is the minimum version: GHSA-phfm-q6fr-wv34 (high) is fixed there. Publishing with the `X-Gotify-Key` header rather than `?token=` keeps the token out of Traefik's access log, which the README says plainly. **ciao** runs HTTP checks on a cron expression each, with TLS expiry per target, and notifies through webhook payloads written by the operator — the placeholders are documented — or mail. Its own authentication is HTTP basic auth and upstream ships it **off**: with an empty user name the interface and the REST API answer anyone who reaches the port, so the stack sets a user name and takes the password from a Docker Secret. `/metrics` is served outside that basic auth and stays off. `SECRET_KEY_BASE` comes from a secret too, because the image otherwise generates a new one on every start and voids every session. Both run `read_only`, `cap_drop: ALL`, `no-new-privileges` and non-root; ciao needs four tmpfs mounts with `mode=1777` because Rails writes its pid and caches and a root-owned tmpfs stops it as uid 1000. ciao's healthcheck asks Ruby, since the image carries no HTTP client and the application has no unauthenticated health route. Verified on the images on 2026-09-23, not yet behind Traefik: hardened starts, the file-based passwords, publishing a Gotify message with an application token and reading it back, ciao refusing without credentials and answering with them, and two ciao checks running on their cron and recording status 200.
- **Two application stacks, batch I of the 2026-09-22 candidates** (`apps/wikijs/`, `apps/shlink/`), both `scaffolded`. **Wiki.js** is the second wiki beside BookStack: pages in PostgreSQL, about twenty authentication modules in the free code, a data volume for caches and uploads. It starts on PostgreSQL although 2.5 accepts four other engines, because 3.0 supports no other — that migration is ahead, and 3.0's own release notes say it is not for production yet. `DB_PASS_FILE` is native and the application trims it. The setup wizard has no password in front of it and makes whoever completes it the administrator, so the router ships VPN-only and the README says to run the setup immediately; the wizard's telemetry checkbox is ticked by default (`telemetry: true` in the shipped bundle) and update checks go to `graph.requarks.io` with no environment variable to stop them, both named in the README. **Shlink** is a URL shortener with no user accounts: the API takes keys, and the initial key has no roles, which in Shlink means it may do everything. Three routers split what has three different audiences — the short links, `/rest`, and the browser client on its own host name — and Traefik's longest-rule-first matching keeps `/rest` on its own access policy even when the redirects are opened. `AUTO_RESOLVE_TITLES` is off, because upstream's default makes the server fetch every long URL that is shortened; `ANONYMIZE_REMOTE_ADDR` stays on; no GeoLite download is attempted without a licence key; and the web client is served **without** `SHLINK_SERVER_API_KEY`, which upstream warns can otherwise be read by anyone who loads the page. Both stacks run `read_only`, `cap_drop: ALL`, `no-new-privileges` and non-root, with their databases on an internal network, and both pin `postgres:18.6-alpine` with `PGDATA` set — PostgreSQL 18 keeps its cluster under `/var/lib/postgresql/<major>` and restart-loops when a volume sits on the old path, which this batch hit and `apps/defectdojo` already worked around. Verified on the images on 2026-09-23, not behind Traefik: the hardened starts, the file-based secrets, Wiki.js' setup and a login through its API, and for Shlink a short URL created with the initial key, a 302 to its target, the recorded visit, and 401 on the API without a key.
