...
- **Backup documentation for `business/`** (10 stacks): the `## Backup` section filled in for every business application, from the compose files rather than from memory — database engine, container, database name and user, the `.secrets/` path, which volumes hold state versus reproducible data, and a copy-pasteable borgmatic block. Several carry a warning that is not obvious from the configuration: OpenSign needs `authentication_database: admin` or the dump authenticates against the wrong database; Zammad's excluded Elasticsearch volume must be reindexed after a restore, and until it finishes the search returns nothing while looking like data loss; Documenso's `cert.p12` validates every signature the instance ever produced; restoring an older Listmonk database can resurrect addresses that unsubscribed since. Invoice Ninja's block is marked unusable as written — it still keeps its database password in `.env` rather than `.secrets/`.
