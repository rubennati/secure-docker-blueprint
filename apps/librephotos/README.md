...
| | |
|---|---|
| **Database** | PostgreSQL · container `librephotos-db` · database `librephotos` · user `librephotos` |
| **Password** | `.secrets/db_pwd.txt` |
| **State** | `./volumes/db` (database) · **`${SCAN_DIRECTORY}`** — the original photos |
| **Reproducible** | `./volumes/protected_media` (thumbnails, faces — regenerated by a rescan) · `./volumes/cache` · `./volumes/logs` |
| **Quiescing** | Not needed for the dump. |

```yaml
postgresql_databases:
    - name: librephotos
      container: librephotos-db
      username: librephotos
      password: "{credential file /srv/docker/apps/librephotos/.secrets/db_pwd.txt}"
```

**`SCAN_DIRECTORY` points outside the stack directory** and holds the only
irreplaceable data here. Everything in `protected_media` is derived from it.

Excluding the derived media saves considerable space, at the cost of a full
rescan after a restore — which on a large library takes hours and pegs the CPU.
Plan for it rather than discovering it.

**Restore order:** database first, then the originals, then the app.
