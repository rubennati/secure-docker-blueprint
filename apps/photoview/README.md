...
| | |
|---|---|
| **Database** | MariaDB · container `photoview-db` · database `photoview` · user `photoview` |
| **Password** | `.secrets/db_pwd.txt` |
| **State** | `./volumes/mariadb` (database) |
| **Reproducible** | `./volumes/media-cache` — thumbnails, regenerated by a rescan |
| **Quiescing** | Not needed. The dump is consistent on its own. |

```yaml
mariadb_databases:
    - name: photoview
      container: photoview-db
      username: photoview
      password: "{credential file /srv/docker/apps/photoview/.secrets/db_pwd.txt}"
```

**`MEDIA_ROOT` is mounted read-only and is not this stack's data.** Photoview
indexes a library it does not own — back that library up wherever it actually
lives, and do not assume this stack's archive covers it.

What this stack owns is the index, the users and the share links. All of it is in
the database.

**Restore order:** database first, then the app. Expect a rescan to repopulate the
cache.
